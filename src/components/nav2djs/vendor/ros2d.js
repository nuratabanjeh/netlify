...
ROS2D.ArrowShape = function(options) {
  ...
  //*/
  /*
  console.log({'name':'ros2d','size':size,'headLen': headLen, 'headWidth': headWidth});
  var polygon = new createjs.Graphics();
  polygon.graphics.beginStroke("black");
  polygon.graphics.setStrokeStyle(strokeSize).moveTo(0, 0).beginStroke(strokeColor).beginFill(fillColor)
               .lineTo(size-headLen, 0).moveTo(size, 0).lineTo(size-headLen, headWidth / 2.0)
               .lineTo(size-headLen, -headWidth / 2.0)//.lineTo(0, 0)
               .closePath().endFill().endStroke();
...
ROS2D.NavigationArrow = function(options) {
  ...
  graphics.lineTo(-size / 2.0, -size / 2.0); // by AMS
  graphics.closePath();
  graphics.endFill();
  graphics.endStroke();
  //*/
...
ROS2D.Viewer = function(options) {
  ...
  var background = options.background || '#transperant';
